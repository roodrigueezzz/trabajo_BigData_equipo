---
title: "El problema ferroviario de España"  #- título
...
format: 
  revealjs:
    ...
    title-slide-attributes: #- atributos para la slide de titulo
      data-background-color: "#313a8f"   #- color de R-ladies
...
```
Hemos decidido  utilizar datos de la web [EUROSTAT](https://ec.europa.eu/eurostat). 
...
</center>![Xd](./imagenes/meme.png){width="400" height="800"}</center>



# ¿Por que este trabajo?

El año pasado para la asignatura de **Política Económica II** se nos instó a leer un libro de forma voluntaria.

En este libro, en el cuál hemos basado y obtenido la idea para este trabajo.

Lo que pretende el autor es hacer una crítica sobre la ineficiencia observada al modelo de infraestructuras español, donde, especialmente los últimos años, se ha visto una política orientada a cumplir otros objetivos que no tienen que ver con los de eficiencia y aumento de la productividad del sistema.

Combinaremos las ideas de este libro con el poder de las herramientas de la programación en **RStudio** que hemos aprendido y desarrollado al largo del curso en la asignatura de Programación y manejo de datos en la era del Big Data, con la finalidad de ver si somos capaces de poner en práctica todo lo aprendido y que nuestro guapo profesor nos apruebe (por favor).

---

<div>
<p style = 'text-align:center;'>
<img src="https://imagessl6.casadellibro.com/a/l/t7/96/9788423346196.jpg" alt="JuveYell" width="550px">
</p>
</div>
...
```{r}
datatable(pasaj_via1, colnames = c("Pais" = "geo", "Nº Pasajeros (miles)" = "Valores_Tren"), options = list(pageLength = 5))
```
...
Estos ofrecen una serie de ventajas, como una mayor velocidad que los trenes de baja velocidad y una   mayor capacidad de carga que los trenes de alta velocidad.        Además, pueden ser una opción de transporte más sostenible y menos contaminante que otros medios de transporte, como aviones o coches.
...
Estos suelen tener una velocidad máxima de más de 250 km/h y pueden ser utilizados tanto en áreas urbanas como rurales. Y en algunos casos, los trenes de alta      velocidad también pueden ser utilizados para el transporte de mercancías a larga distancia.
...
Grafico_1b <- ggplot(Tren_Pasaj) +
  ...
  ggthemes::theme_base() +
  facet_wrap(vars(geo)) + theme(axis.text.y = element_text(size = 8), axis.text.x = element_text(size = 8))
...
Grafico_2b <- ggplot(Tren_Mxp) +
  ...
  ggthemes::theme_base() +
  facet_wrap(vars(geo)) + theme(axis.text.x = element_text(size = 8),
...
Para acabar esta parte, vamos a señalar una particularidad que sucede si observamos a España de cerca
...
En este segundo apartado, tras observar como España es de los países Europeos con mas vías a su disposicion, queremos comprobar si se le da mas uso, a parte del trasnporte de pasajeros, para sacarle mayor rentabilidad.
...
p1 <-ggplot(paises_3) +
  ...
  labs(
    ...
    title = "Uso del tren para el transporte de mercancías"
  ) +
  ggthemes::theme_base() + theme(plot.title = element_text(size = 12))
...
p3 <-ggplot(paises_5) +
  ...
  labs(
    ...
    title = "Uso del camión para el transporte de mercancías"
  ) +
  ggthemes::theme_base() + theme(plot.title = element_text(size = 12))
...
<font size="5"> Se observa que la mercancía transportada en España mediante el ferrocarril es más más baja que en Europa llegando a ser superado por Suecia. Esto viene determinado por la preferencia en españa por el transporte en camión, que es un medio de transporte más flexible y  que permite el transporte "puerta a puerta". Además este bajo uso del ferrocarril, también viene consicionado por el ancho de vía español (igual que el inglés)  que difiere del del resto de Europa, lo que impide un tránsito fluido una vez se quiere cruzar la frontera. </font> 


# EL CASO ESPAÑOL

## Análisis de los tipos de vía
...
No obstante, como se puede observar, no hemos dispuesto las leyendas para identificar cada país, dado que sería un ejercicio totalmente sinsentido, ya que hay países que, simplemente por su mayor superficie territorial cuentan con mayor kilometraje de vía.

Por tanto, lo que haremos a continuación será obtener los datos de superficie en km2 del Banco Mundial.
...
Como podemos observar en los gráficos de [graf_media_doble](#vías-de-media-velocidad) y [graf_alta_doble](#vías-de-alta-velocidad) comparado con [Grafico_España](#tercer-gráfico) podemos observar claramente como España, pese a ser el país europeo con más vías de alta velocidad respecto al total de km de Europa, los pasajeros siguen sin ser muy elevados. De hecho, es notablemente mayor la afluencia en baja velocidad, e incluso en media también lo es.
...
Además, hablando del transporte de mercancías, [p4](#segundo-gráfico-1) , observamos que España es un país que transporta muchas más mercancías por carretera que por tren en pesos relatives de km de vía. Así, España es la que menos rendimiento extrae de su extensa infraestructura.
...
<div>
<p style = 'text-align:center;'>
<img src="https://i.ibb.co/tBqtRfP/meme2.jpg" alt="JuveYell" width="400px">
</p>
</div>

# ¡¡Y final!! ¡¡Muchas gracias a todos!!
